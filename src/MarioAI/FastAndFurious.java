...
public class FastAndFurious extends KeyAdapter implements Agent {
	...
	public boolean[] getAction(Environment observation) {
		...
		if (tickCount == 30) {
			...
			if ((world.hasGoalNodesChanged()
					...
					|| enemyPredictor.hasNewEnemySpawned() || pathCreator.getBestPath() == null)
					&& marioController.canUpdatePath || !pathCreator.isRunning) {
				/*
				 * pathCreator.syncWithRealWorld(world,
				 * enemyPredictor); findPath(observation);
				 */
				/*
				 * if (world.hasGoalNodesChanged()) {
				 * System.out.println("Reason: World"); } if
				 * (MarioControls.isPathInvalid(observation,
				 * pathCreator.getBestPath())) {
	...
	public void findPath(Environment observation) {
		...
		pathCreator.blockingFindPath(observation, world.getMarioNode(observation), world.getGoalNodes(0),
				marioController.getXVelocity(), enemyPredictor, marioHeight, world);
