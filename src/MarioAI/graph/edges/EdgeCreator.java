...
public class EdgeCreator {
	...
	private boolean getJumpStraightUpEdges(Node startingNode, int nodeColoumn,
		...
		for (int jumpHeight = (int) 1; jumpHeight <= MAX_JUMP_HEIGHT; jumpHeight++) {
			...
			if (isHittingWallOrGroundUpwards(nodeColoumn,
				...
				// Else will Mario be able to jump up to this
				// height, to the current target:
				listOfEdges.add(new JumpingEdge(startingNode, currentLandingPosition,
						startingNode.y + jumpHeight));
			}
		}
	...
	public boolean getRunningReachableEdges(Node startingNode, int nodeColoumn, List<DirectedEdge> listOfEdges) {
		...
		// Run to the right:
		if (nodeColoumn + 1 < GRID_WIDTH) { // Not at the rightmost
							// block in the view.
			listOfEdges.add(new RunningEdge(startingNode,
					observationGraph[nodeColoumn + 1][startingNode.y]));
		} else
			foundAllEdges = false;
	...
	public boolean jumpAlongPolynomial(Node startingNode, int nodeColoumn, JumpingEdge polynomial,
		...
		boolean isPastTopPointColumn = polynomial.isPastTopPoint(direction,
		...
		while (isWithinView(currentXPosition + xPositionOffsetForJump)) {
			...
			if (isPastTopPointColumn && !hasAlreadyPassedTopPoint) {
				if ((polynomial.getTopPointX() < currentXPosition && !direction.isLeftType())) { // rightwards!
					currentXPosition--; // The toppoint was
								// in the
								// current block
								// (and not
								// ending
								// there).
					// Therefore the downward going part of
					// that block needs to be checked.
					// See more detailed explanation in the
					// report.
			...
			if (collisionDetection == Collision.HIT_WALL) {
				currentXPosition = currentXPosition
						+ direction.getOppositeDirection().getHorizontalDirectionAsInt();
				xPositionOffsetForJump = xPositionOffsetForJump
						+ direction.getHorizontalDirectionAsInt();
	...
	public Collision ascendingFunction(float formerLowerYPosition, float bound, int currentXPosition,
		...
		for (float y = formerLowerYPosition; y >= Math.max(bound, 0) - 0.99; y--) {
			...
			final Collision lowerFacingMarioCorner = lowerFacingCornerCollision(y, currentXPosition,
					collisionDetection, direction);
			final Collision upperFacingMarioCorner = upperFacingCornerCollision(isHittingWall, y,
					formerLowerYPosition, currentXPosition, direction);
			final Collision upperOppositeMarioCorner = upperOppositeCornerCollision(y, currentXPosition,
					direction);
			final Collision middleFacingMarioCorner = middleFacingCornerCollision(y, currentXPosition,
					direction);
			// As it is ascending to the right, only worry about the
			// two corners to the right/left,
			// and maybe the middle right/left part, if Mario has
			// height 2.
			if (upperOppositeMarioCorner == Collision.HIT_CEILING
				...
				if (addEdges) {
					listOfEdges.add(new JumpingEdge(startingPosition,
							observationGraph[currentXPosition][(int) y], polynomial));
				}
				break;
	...
	public Collision descendingPolynomial(float formerLowerYPosition, float bound, int currentXPosition,
		...
		for (float y = formerLowerYPosition; y <= Math.min(bound, GRID_HEIGHT - 1) + 0.99; y++) {
			...
			final Collision upperFacingMarioCorner = upperFacingCornerCollision(isHittingWall, y,
					formerLowerYPosition, currentXPosition, direction);
			final Collision lowerOppositeMarioCorner = lowerOppositeCornerCollision(y, currentXPosition,
					direction);
			// As it is descending to the right, only worry about
			// the two corners to the right, and the left lower one.
			// TODO change this so that walls are prioritized
			// correctly
			// TODO discuss why it is HIT_NOTHING or HIT_WALL for
			// upperFacingMarioCorner.
			if ((upperFacingMarioCorner == Collision.HIT_NOTHING
				...
				if (lowerFacingMarioCorner == Collision.HIT_GROUND) {
					...
					final int groundXPos = currentXPosition + direction.getOppositeDirection()
							.getHorizontalDirectionAsInt();
					listOfEdges.add(new JumpingEdge(startingPosition,
							observationGraph[groundXPos][(int) y], polynomial));
				}
				break;
			} else if (upperFacingMarioCorner == Collision.HIT_WALL
					|| lowerFacingMarioCorner == Collision.HIT_WALL) {
	...
	private boolean isHittingWallOrGroundUpwards(int xPosition, int yPosition) {
		// Being out of the level matrix does not constitute as hitting
		// something
		// Needs yPosition-1, as else one will get an immediate
		// collision with the ground, as mario starts on it.
	...
	private boolean canMarioStandThere(int coloumn, int row) {
		return 0 < row && row < GRID_HEIGHT && isOnSolidGround(row, coloumn)
				&& !isSolid(observationGraph[coloumn][row - 1]);
	}

	private boolean isOnSolidGround(int row, int coloumn) {
		return observationGraph[coloumn][row] != null; // TODO Fix in
								// general.
	}

	private boolean isAir(int coloumn, int row) {
		// return node != null;
		return !(0 <= row && row < GRID_HEIGHT && 0 <= coloumn && coloumn < GRID_WIDTH)
				|| observationGraph[coloumn][row] == null;// TODO(*)
										// Fix
	}

	private Collision lowerFacingCornerCollision(float y, int currentXPosition, Collision collisionDetection,
			JumpDirection direction) {
		if (direction.isUpwardsType()) {
			// In the case one is going upwards, one should not
			// check for any other type of collisions,
			// than those originating from wall collisions.
			// TODO check and discuss if ceiling the result is
			// correct.
			if (isHittingWallOrGroundUpwards(currentXPosition, (int) Math.ceil(y))) { // If
													// it
													// is
													// hitting
													// the
			...
			// One must be going downwards, and there should be
			// checked for collisions:
			// TODO check and discuss if ceiling the result is
			// correct.
			if (isHittingWallOrGroundDownwards(currentXPosition, (int) Math.floor(y))) {
				// If this corner is hitting something, then
				// there are two possibilities: either it is the
				// ground or a wall.
				// If it is the ground, then Mario can stand
				// there, and if it is a wall, it is not
				// possible.
	...
	private Collision upperFacingCornerCollision(boolean isHittingWall, float y, float formerLowerYPosition,
			int currentXPosition, JumpDirection direction) {
		// TODO likely error here with the direction.
		if (direction.isUpwardsType()) {
			// If mario is going upwards, one needs to check for
			// ceiling collisions and the wall collisions,
			// and not whether he hits the ground. This will be
			// registered by the lower part.
			// TODO check and discuss if ceiling the result is
			// correct.
			if (isHittingWallOrGroundUpwards(currentXPosition, (int) Math.ceil(y - MARIO_HEIGHT))) {
				if (y == formerLowerYPosition) {
					return Collision.HIT_WALL;
				} else if (!isHittingWall) {
					// TODO make.
					// hitWallOrGround(listOfEdges,
					// currentXPosition,y);
	...
	private Collision upperOppositeCornerCollision(float y, int currentXPosition, JumpDirection direction) {
		currentXPosition += direction.getOppositeDirection().getHorizontalDirectionAsInt();

		// If Mario is going upwards, and since this is the opposite
		// corner of the way he is going,
	...
	private Collision lowerOppositeCornerCollision(float y, int currentXPosition, JumpDirection direction) {
		currentXPosition += direction.getOppositeDirection().getHorizontalDirectionAsInt();

		// It should check if Mario hits the ground: it can't be the
		// wall, as Mario is going in the way opposite to this corner.
		// TODO check and discuss if ceiling the result is correct.
		if (isHittingWallOrGroundDownwards(currentXPosition, (int) Math.floor(y))) {
			if (canMarioStandThere(currentXPosition, (int) Math.floor(y))) {
				return Collision.HIT_GROUND;
				...
				// throw new Error("Logic error on corner
				// collision detection");
			}
		} else {
			return Collision.HIT_NOTHING;
	...
	private Collision middleFacingCornerCollision(float y, int currentXPosition, JumpDirection direction) {
		// TODO fix this, doens't work properly
		// Mario must have height 2, before this is relevant:
		if (MARIO_HEIGHT >= 1.5) {
			// Some blocks can be passed through going upwards, not
			// downwards, so two cases are necessary:
			// TODO check if it is nessecary(*)
			if ((direction.isUpwardsType() && isHittingWallOrGroundUpwards(currentXPosition,
					(int) Math.ceil(y - MARIO_HEIGHT / 2)))
					|| (!direction.isUpwardsType() && isHittingWallOrGroundDownwards(
							currentXPosition, (int) Math.ceil(y - MARIO_HEIGHT / 2)))) { // If
															// it
															// is
															// hitting
															// the
			...
			return Collision.HIT_NOTHING; // Do not include it,
							// which means, take it
							// as having no
							// collisions
		// Can only lead to a wall collision, and this happens, if and
		// only if it is in a wall:
	}
