...
public class Enemy extends Sprite {
	...
	public Enemy(Enemy enemy) {
		...
		switch (enemy.type) {
		case ENEMY_RED_KOOPA:
			k = (byte) (4 + ((enemy.winged) ? 1 : 0));
			break;
		case ENEMY_GREEN_KOOPA:
			k = (byte) (6 + ((enemy.winged) ? 1 : 0));
			break;
		case ENEMY_GOOMBA:
			k = (byte) (2 + ((enemy.winged) ? 1 : 0));
			break;
		case ENEMY_FLOWER:
			k = (byte) (11);
			break;
		case ENEMY_SPIKY:
			k = (byte) (9 + ((enemy.winged) ? 1 : 0));
		}
		kind = k;
		this.type = enemy.type;
		// sheet = Art.enemies;
		this.winged = enemy.winged;

		this.x = enemy.x;
		this.y = enemy.y;
		this.mapX = enemy.mapX;
		this.mapY = enemy.mapY;

		this.world = enemy.world;
		xPicO = 8;
		yPicO = 31;

		avoidCliffs = enemy.type == Enemy.ENEMY_RED_KOOPA;

		noFireballDeath = enemy.type == Enemy.ENEMY_SPIKY;
	...
	// {
	// xFlipPic = !xFlipPic;
	// og.drawImage(sheet[wingTime / 4 % 2][4], xPixel + (xFlipPic ? wPic : 0) + (xFlipPic ? 10 : -10), yPixel + (yFlipPic ?
	// hPic : 0) - 8, xFlipPic ? -wPic : wPic, yFlipPic ? -hPic : hPic, null);
	// xFlipPic = !xFlipPic;
	// }
	// }
	//
	...
	// {
	// og.drawImage(sheet[wingTime / 4 % 2][4], xPixel + (xFlipPic ? wPic : 0) + (xFlipPic ? 10 : -10), yPixel + (yFlipPic ?
	// hPic : 0) - 10, xFlipPic ? -wPic : wPic, yFlipPic ? -hPic : hPic, null);
	// }
	// else
	// {
	// og.drawImage(sheet[wingTime / 4 % 2][4], xPixel + (xFlipPic ? wPic : 0) + (xFlipPic ? 10 : -10), yPixel + (yFlipPic ?
