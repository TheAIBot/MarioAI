...
public class LevelScene extends Scene implements SpriteContext {
	...
	public LevelScene(GraphicsConfiguration graphicsConfiguration, MarioComponent renderer, long seed,
			int levelDifficulty, int type, int levelLength, int timeLimit) {
	...
	private byte ZLevelMapElementGeneralization(byte el, int ZLevel) {
		...
		case (1):
			...
			case (9):
				return -10; // border, cannot pass through, can
						// stand on
			// case(9):
			...
			case (-76):
			case (-74):
				return -11; // half-border, can jump through
	...
	public byte[][] enemiesObservation(int ZLevel) {
		...
		for (Sprite sprite : sprites) {
			if (sprite.kind == mario.kind)
				continue;
			if (sprite.mapX >= 0 && sprite.mapX > MarioXInMap - Environment.HalfObsWidth
					&& sprite.mapX < MarioXInMap + Environment.HalfObsWidth && sprite.mapY >= 0
	...
	public byte[][] mergedObservation(int ZLevelScene, int ZLevelEnemies) {
		...
		for (Sprite sprite : sprites) {
			...
			if (sprite.mapX >= 0 && sprite.mapX > MarioXInMap - Environment.HalfObsWidth
				...
				int obsX = sprite.mapY - MarioYInMap + Environment.HalfObsHeight;
				int obsY = sprite.mapX - MarioXInMap + Environment.HalfObsWidth;
				// quick fix TODO: handle this in more general
				// way.
				if (ret[obsX][obsY] != 14) {
	...
	public String bitmapLevelObservation(int ZLevel) {
		...
		for (int y = MarioYInMap - Environment.HalfObsHeight, obsX = 0; y < MarioYInMap
			...
			for (int x = MarioXInMap - Environment.HalfObsWidth, obsY = 0; x < MarioXInMap
				...
				++totalBits;
				if (bitCounter > 15) {
					// update a symbol and store the current
					// one
					ret += block;
					++totalBytes;
					block = 0;
	...
	public List<String> LevelSceneAroundMarioASCII(boolean Enemies, boolean LevelMap, boolean mergedObservationFlag,
		...
		// OutputStreamWriter(os));// bw.write("\nTotal world width = "
		// + level.width);
		...
		if (level != null && mario != null) {
			...
			if (mergedObservationFlag) {
				...
				// "===========\nAll objects: (LevelScene[x,y],
				// Sprite[x,y])==/* Mario ~> MM */=====\n");
				// for (int x = 0; x < levelScene.length; ++x)
				// {
				// String tmpData = "";
				// for (int y = 0; y < levelScene[0].length;
				// ++y)
				// tmpData += "(" + levelScene[x][y] + "," +
				// enemiesObservation[x][y] + ")";
				// ret.add(tmpData);
				// }
	...
	public void initLoadedLevel(Level level) {
		try {
			Level.loadBehaviors(new DataInputStream(
					LevelScene.class.getResourceAsStream("resources/tiles.dat")));
		} catch (IOException e) {
			e.printStackTrace();
			System.exit(0);
		}
	...
	public void render(Graphics g, float alpha) {
		...
		if (GlobalOptions.MarioAlwaysInCenter) {
			...
			// int xCam = (int) (xCamO + (this.xCam - xCamO) *
			// alpha);
			// int yCam = (int) (yCamO + (this.yCam - yCamO) *
			// alpha);
			if (xCam < 0)
				xCam = 0;
			if (yCam < 0)
		...
		for (Sprite sprite : sprites) {
			if (sprite.layer == 1)
				sprite.render(g, alpha);
			if (mario.cheatKeys[Mario.KEY_DUMP_CURRENT_WORLD] && sprite.mapX >= 0
					&& sprite.mapX < level.observation.length && sprite.mapY >= 0
		...
		// drawStringDropShadow(g, "MARIO: " + df.format(Mario.lives),
		// 0, 0, 7);
		...
		if (GlobalOptions.Labels) {
			g.drawString("xCam: " + xCam + "yCam: " + yCam, 70 * Art.SIZE_MULTIPLIER,
					40 * Art.SIZE_MULTIPLIER);
			g.drawString("x : " + mario.x + "y: " + mario.y, 70 * Art.SIZE_MULTIPLIER,
					50 * Art.SIZE_MULTIPLIER);
			g.drawString("xOld : " + mario.xOld + "yOld: " + mario.yOld, 70 * Art.SIZE_MULTIPLIER,
					60 * Art.SIZE_MULTIPLIER);
		}
	...
	private void drawProgress(Graphics g) {
		...
		progress_str += "M";
		try {
			drawStringDropShadow(g, entirePathStr.substring(progress_str.length()), progress_str.length(),
					28 / 2, 0);
		} catch (StringIndexOutOfBoundsException e) {
			// System.err.println("warning: progress line
			// inaccuracy");
		}
	...
	public float getX(float alpha) {
		int xCam = (int) (mario.xOld + (mario.x - mario.xOld) * alpha) - 160;
		// int yCam = (int) (mario.yOld + (mario.y - mario.yOld) *
		// alpha) - 120;
