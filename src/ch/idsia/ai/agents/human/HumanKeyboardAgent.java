 ...
 * 12:19:49 AM Package: ch.idsia.ai.agents.ai;
 */
public class HumanKeyboardAgent extends KeyAdapter implements Agent {
	...
	public void reset() {
		// Just check you keyboard. Especially arrow buttons and 'A' and
		// 'S'!
	...
	public boolean[] getAction(Environment observation) {
		...
		System.out.println(
				observation.getMarioFloatPos()[0] / 16 + " " + observation.getMarioFloatPos()[1] / 16);
		// System.out.println(observation.getCompleteObservation().length
		// + " " + observation.getCompleteObservation()[0].length);
		/*
		 * byte[][] level = observation.getCompleteObservation(); for
		 * (int x = 0; x < level.length; x++) { for (int y = 0; y <
		 * level[0].length; y++) { System.out.print((char)(level[x][y] +
		 * 50)); } System.out.println(""); } System.out.println();
		 */
		return Action;
	}
